...
router.post("/car/create", async (req, res) => {
  ...
  try {
    ...
    // Update car rating
    const newCarRating = calculateNewRating(
      car.rating,
      car.numberOfReviews,
      rating
    );
...
router.put("/car/:id", async (req, res) => {
  ...
  try {
    ...
    if (car) {
      car.rating =
        (car.rating * car.numberOfReviews - oldRating + rating) /
        car.numberOfReviews;
      await car.save({ session });
      ...
      if (vendor) {
        vendor.rating =
          (vendor.rating * vendor.numberOfReviews - oldRating + rating) /
          vendor.numberOfReviews;
        await vendor.save({ session });
      }
    }
...
router.delete("/car/:id", async (req, res) => {
  ...
  try {
    ...
    if (car && car.numberOfReviews > 1) {
      car.rating =
        (car.rating * car.numberOfReviews - oldRating) /
        (car.numberOfReviews - 1);
      car.numberOfReviews -= 1;
      await car.save({ session });
      ...
      if (vendor && vendor.numberOfReviews > 1) {
        vendor.rating =
          (vendor.rating * vendor.numberOfReviews - oldRating) /
          (vendor.numberOfReviews - 1);
        vendor.numberOfReviews -= 1;
        await vendor.save({ session });
      }
...
router.post("/catering/create", async (req, res) => {
  ...
  try {
    ...
    const newCateringRating = calculateNewRating(
      catering.rating,
      catering.numberOfReviews,
      rating
    );
...
router.put("/catering/:id", async (req, res) => {
  ...
  try {
    ...
    if (catering) {
      catering.rating =
        (catering.rating * catering.numberOfReviews - oldRating + rating) /
        catering.numberOfReviews;
      await catering.save({ session });
...
router.delete("/catering/:id", async (req, res) => {
  ...
  try {
    ...
    if (catering && catering.numberOfReviews > 1) {
      catering.rating =
        (catering.rating * catering.numberOfReviews - oldRating) /
        (catering.numberOfReviews - 1);
      catering.numberOfReviews -= 1;
      await catering.save({ session });
...
router.post("/hall/create", async (req, res) => {
  ...
  try {
    ...
    // Update hall rating
    const newHallRating = calculateNewRating(
      hall.rating,
      hall.numberOfReviews,
      rating
    );
...
router.put("/hall/:id", async (req, res) => {
  ...
  try {
    ...
    if (hall) {
      hall.rating =
        (hall.rating * hall.numberOfReviews - oldRating + rating) /
        hall.numberOfReviews;
      await hall.save({ session });
...
router.delete("/hall/:id", async (req, res) => {
  ...
  try {
    ...
    if (hall && hall.numberOfReviews > 1) {
      hall.rating =
        (hall.rating * hall.numberOfReviews - oldRating) /
        (hall.numberOfReviews - 1);
      hall.numberOfReviews -= 1;
      await hall.save({ session });
...
router.post("/decoration/create", async (req, res) => {
  ...
  try {
    ...
    const newDecorationRating = calculateNewRating(
      decoration.rating,
      decoration.numberOfReviews,
      rating
    );
...
router.put("/decoration/:id", async (req, res) => {
  ...
  try {
    ...
    if (decoration) {
      decoration.rating =
        (decoration.rating * decoration.numberOfReviews - oldRating + rating) /
        decoration.numberOfReviews;
      await decoration.save({ session });
...
router.delete("/decoration/:id", async (req, res) => {
  ...
  try {
    ...
    if (decoration && decoration.numberOfReviews > 1) {
      decoration.rating =
        (decoration.rating * decoration.numberOfReviews - oldRating) /
        (decoration.numberOfReviews - 1);
      decoration.numberOfReviews -= 1;
      await decoration.save({ session });
