...
function calculateUpdatedRating(totalRating, numberOfReviews, oldRating, newRating) {
  return (totalRating * numberOfReviews - oldRating + newRating) / numberOfReviews;
}
...
router.get('/vendor', authVendor, async (req, res) => {
  try {
    ...
    const reviews = await Review.aggregate([
      ...
      { $unwind: '$user' },
      { $project: { rating: 1, comment: 1, serviceType: 1, createdAt: 1, 'user.name': 1, 'user.email': 1, 'user.profileImage': 1 } }
    ])
...
router.post('/:type/create', async (req, res) => {
  ...
  try {
    ...
    service.rating = calculateNewRating(service.rating || 0, service.numberOfReviews || 0, rating)
...
router.put('/:type/:id', async (req, res) => {
  ...
  try {
    ...
    if (service) {
      service.rating = calculateUpdatedRating(service.rating, service.numberOfReviews, oldRating, rating)
      await service.save({ session })

      const vendor = await Vendor.findById(service.vendorId)
      if (vendor) {
        vendor.rating = calculateUpdatedRating(vendor.rating, vendor.numberOfReviews, oldRating, rating)
        await vendor.save({ session })
      }
    }
...
router.delete('/:type/:id', async (req, res) => {
  ...
  try {
    ...
    if (service && service.numberOfReviews > 1) {
      service.rating = calculateRemovedRating(service.rating, service.numberOfReviews, oldRating)
      service.numberOfReviews -= 1
      await service.save({ session })

      const vendor = await Vendor.findById(service.vendorId)
      if (vendor && vendor.numberOfReviews > 1) {
        vendor.rating = calculateRemovedRating(vendor.rating, vendor.numberOfReviews, oldRating)
