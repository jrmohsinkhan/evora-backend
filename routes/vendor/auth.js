...
router.get('/google/vendor/callback',
    passport.authenticate('google-vendor', { session: false, failureRedirect: '/' }),
    (req, res) => {
...
router.post('/register', async (req, res) => {
    const { name, email, password, confirmPassword, businessName, phone, address } = req.body;  // Added confirmPassword
    ...
    try {
        ...
        if (vendor) return res.status(400).json({ msg: 'User already exists' });
...
router.post('/verify-otp', otpLimiter, async (req, res) => {
    ...
    try {
        ...
        if (!vendor) return res.status(400).json({ msg: 'User not found' });
        if (vendor.isVerified) return res.status(400).json({ msg: 'User already verified' });
